...
def load_model(path=None):
    ...
    if path is None:
        ...
        model_file = max(list_of_files, key=os.path.getmtime)
    else:
        model_file = path
...
class DataLogger:
    ...
    def progress(self):
        ...
        """
        curr_time = time.time()
        steps = int((self.time_limit + 2) / (curr_time - self.loop_time))

        if curr_time > self.time:
            progress = int((curr_time - self.time) / (curr_time - self.loop_time))
        else:
            progress = 0

        self.loop_time = curr_time
